...
const DUMMY_CHAT_SETTINGS: ChatSettings = {
  ...
  assistantIntroductions: 'How can I help you.',
  model: null,
  seed: null,
  temperature: null,
  top_p: null
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 px-8 pt-6 pb-8 mb-4">
          ...
          <div className="mb-4">
            <FormLabel readOnly={readOnly} label={t('instructions-header')} htmlFor="instructions"
                       value={formData.systemInstructions || t('non-applicable')}></FormLabel>
            {readOnly ?
                <p className="text-gray-700 dark:text-gray-300">{formData.systemInstructions || t('non-applicable')}</p> :
                <textarea
                    id="instructions"
          ...
          <div className="mb-4">
            <FormLabel readOnly={readOnly} label={t('introductions-header')} htmlFor="introductions"
                       value={formData.assistantIntroductions || t('non-applicable')}></FormLabel>
            {readOnly ?
                <p className="text-gray-700 dark:text-gray-300">{formData.assistantIntroductions || t('non-applicable')}</p> :
                <textarea
                    id="introductions"
                    value={formData.assistantIntroductions}
                    name="introductions"
                    onChange={handleInputChange}
                    className="resize-y border rounded-sm overflow-y-auto h-56 w-full max-h-[60vh] md:max-h-[calc(100vh-300px)] shadow-sm appearance-none py-2 px-3 text-gray-700 dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 leading-tight focus:outline-hidden focus:shadow-outline"
                ></textarea>}
