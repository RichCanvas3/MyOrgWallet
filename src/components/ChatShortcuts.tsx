      ...
      <div>
        {chatSettings.map((setting) => (
            <Link to={`/g/${setting.id}`} key={setting.id}
                  className="flex py-3 px-3 items-center gap-3 relative rounded-md cursor-pointer break-all hover:bg-gray-100 ">
              <div className="bg-white  overflow-hidden rounded-full">
                {setting.icon?.data ? (
                    <img src={setting.icon.data} alt="" className="bg-gray-100"
              ...
              <span
                  className="flex-1 overflow-hidden whitespace-nowrap text-ellipsis max-h-5 break-all relative text-gray-800">
      {setting.name}
    </span>
            </Link>
        ))}
