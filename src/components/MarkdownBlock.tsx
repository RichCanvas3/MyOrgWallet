...
const MarkdownBlock: React.FC<ChatBlockProps> = ({markdown, role, loading}) => {
  ...
  function codeBlock({node, className, children, ...props}: any) {
    ...
    return isInline ? (
      ...
      <div className="relative border border-gray-200 rounded-md codeBlockContainer ">
        <div
          className="flex items-center text-gray-900 bg-gray-200 px-4 py-2 text-xs font-sans justify-between rounded-t-md">
          <span>{language}</span>
        </div>
        <div className="sticky top-9 md:top-[5.75rem]">
          <div className="absolute bottom-0 right-2 flex h-8 items-center">
            <CopyButton className="rounded-sm bg-gray-200" text={children}/>
          </div>
        </div>
