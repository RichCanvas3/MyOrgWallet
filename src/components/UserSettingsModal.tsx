...
import EditableIntroductions from './EditableIntroductions';
...
enum Tab {
  ...
  INTRODUCTIONS_TAB = "Introductions",
  SPEECH_TAB = "Speech",
  STORAGE_TAB = "Storage",
}
...
const UserSettingsModal: React.FC<UserSettingsModalProps> = ({isVisible, onClose}) => {
  ...
  const editableIntroductionsRef = useRef<{ getCurrentValue: () => string }>(null);
  ...
  const handleDeleteAllAttestations = async () => {
    showConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          await AttestationService.deleteAllAttestations();
  ...
  const handleClose = () => {
    ...
    setUserSettings({...userSettings, systemInstructions: currentInstructions || ''});

    const currentIntroductions = editableIntroductionsRef.current?.getCurrentValue();
    setUserSettings({...userSettings, assistantIntroductions: currentIntroductions || ''});
            ...
            <div ref={dialogRef}
              ...
              <div id='user-settings-content' className="flex flex-1">
                <div className="border-r border-gray-200 flex flex-col">
                  ...
                  <div
                      className={`cursor-pointer p-4 flex items-center ${activeTab === Tab.INTRODUCTIONS_TAB ? 'bg-gray-200 dark:bg-gray-700' : ''}`}
                      onClick={() => setActiveTab(Tab.INTRODUCTIONS_TAB)}>
                    <DocumentTextIcon className="w-4 h-4 mr-3"
                                      aria-hidden="true"/>{t('introductions-tab')}
                  </div>
                  <div
                ...
                <div className="flex-1 p-4 flex flex-col">
                  ...
                  <div
                    ...
                    <div
                      ...
                      <EditableInstructions
                          ...
                          initialValue={userSettings.systemInstructions}
                          placeholder={OPENAI_DEFAULT_SYSTEM_PROMPT}
                          onChange={(text) => {
                            setUserSettings({...userSettings, systemInstructions: text});
                          }}
                  ...
                  <div
                      className={`${activeTab === Tab.INTRODUCTIONS_TAB ? 'flex flex-col flex-1' : 'hidden'}`}>
                    <div
                        className="flex flex-col flex-1 border-b border-token-border-light pb-3 last-of-type:border-b-0">
                      <EditableIntroductions
                          ref={editableIntroductionsRef}
                          initialValue={userSettings.assistantIntroductions}
                          placeholder={OPENAI_DEFAULT_ASSISTANT_PROMPT}
                          onChange={(text) => {
                            setUserSettings({...userSettings, assistantIntroductions: text});
                          }}
                          className="flex flex-col h-full"
                      />
                    </div>
                  ...
                  <div className={`${activeTab === Tab.STORAGE_TAB ? 'flex flex-col flex-1' : 'hidden'}`}>
                    ...
                    <div className="setting-panel">
                      <p>Chats and Attestations are stored locally in your browser's IndexedDB.</p>
                      <p>
