...
app.get('/linkedin-callback', async (req, res) => {
    ...
    try {
      ...
      //let j = JSON.stringify(objToSortedArray(record))
      let j = "hello world"
      ...
      // Respond with the access token (for demonstration purposes)
  ...
  app.get('/x-callback', async (req, res) => {
    ...
    const { code, verifier } = req.query;  // The authorization code from X
    ...
    try {
      ...
      const response2 = await axios.get('https://api.x.com/2/users/me?user.fields=id,name,username,created_at,description,entities,location,pinned_tweet_id,profile_image_url,protected,public_metrics,url,verified,verified_type,withheld', {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
        }
  ...
  app.get('/shopify-callback', async (req, res) => {
    ...
    const { code } = req.query;  // The authorization code from X
